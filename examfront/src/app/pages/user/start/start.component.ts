...
export class StartComponent implements OnInit {
  ...
  startTimer() {
    this.timer = setInterval(() => {
      if (this.timeLeft <= 0) {
        clearInterval(this.timer);
        this.calculateResult(); // auto submit
      } else {
        this.timeLeft--;
  ...
  calculateResult() {
    ...
    console.log('Sending to evalQuiz:', JSON.stringify(this.questions.map(q => ({
      quesId: q.quesId,
      givenAnswers: q.givenAnswers,
      answer: q.answer
    }))));
    ...
    this._question.evalQuiz(this.questions).subscribe(
      ...
      (error) => {
        console.log(error);
        Swal.fire('Error', 'Failed to evaluate quiz. Please try again.', 'error');
      }
    );
  }
}